...
### Phase 4.5: Anti-bot / Robot Fail Fallback (필수)

`robot check`, `access denied`, `captcha`, `cloudflare`, `403`가 나오면 아래 순서대로 재시도:

1. **Direct HTTP 재시도 (저비용)**
   - careers URL, ATS URL 각각 `curl -I`/`curl -L`로 상태 확인
   - `jobs.lever.co`, `boards.greenhouse.io`, `jobs.ashbyhq.com`의 **specific job URL** 우선

2. **Playwright MCP fallback (고비용)**
   - 실제 브라우저로 페이지 진입 후 DOM 스냅샷/텍스트 추출
   - JS 렌더링/지연 로딩 케이스를 우선 해결

3. **ATS 직접 엔드포인트 확인**
   - Greenhouse/Lever/Ashby의 공개 job board에서 role title 재탐색
   - generic careers 페이지가 아닌 job-specific URL만 채택

4. **Cross-source 보수적 확정**
   - Company careers + ATS/LinkedIn 중 최소 1개 이상에서 같은 role 확인 시만 유지
   - 충돌 시 role을 추가하지 말고 `needs_review`로 남김

5. **절대 금지**
   - robot fail 상태에서 추정 URL 생성
   - generic careers URL을 openRoles.url로 저장
   - 검증 실패 role을 "열려 있음"으로 단정
...
```
1. WebFetch/HTTP 재시도
2. Playwright MCP로 fallback
3. ATS 직접 엔드포인트 확인 (Greenhouse/Lever/Ashby)
4. 그래도 불가하면 role 추가하지 말고 needs_review로 기록
```
