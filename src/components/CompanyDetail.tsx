...
function PlatformIcon({ platform }: { platform: string }) {
  ...
  return <span className="text-xs">{icons[platform] || 'L'}</span>;
}
            ...
            {company.openRoles.length > 0 ? (
              <div className="space-y-4">
                {company.openRoles.map((role, i) => (
                  <div key={i} className="card p-5 border-[var(--success)]">
                    {/* Header */}
                    <div className="mb-3">
                      <a
                        href={role.url}
                        target="_blank"
                        rel="noopener noreferrer"
                        className="font-semibold hover:text-[var(--accent-light)] text-base"
                      >
                        {role.title} ↗
                      </a>
                      <div className="flex flex-wrap items-center gap-2 mt-1">
                        ...
                        {role.level && (
                          <span className="badge text-xs">{role.level}</span>
                        )}
                        {role.team && (
                          <span className="badge badge-accent text-xs">{role.team}</span>
                        )}
                      </div>
                      {role.compensation && (
                    ...
                    {/* Why Interesting */}
                    {role.whyInteresting && (
                      <div className="mb-4 p-3 bg-[var(--success)]/10 rounded-lg border border-[var(--success)]/30">
                        <div className="text-xs font-medium text-[var(--success)] mb-1">Why This Role is Interesting</div>
                    ...
                    {role.responsibilities && role.responsibilities.length > 0 && (
                      <div className="mb-4">
                        <div className="text-xs font-medium text-[var(--muted)] uppercase tracking-wide mb-2">What You'll Do</div>
                    ...
                    {role.requirements && role.requirements.length > 0 && (
                      <div className="mb-4">
                        <div className="text-xs font-medium text-[var(--muted)] uppercase tracking-wide mb-2">Requirements</div>
                        <ul className="space-y-1">
                    ...
                    {role.preferred && role.preferred.length > 0 && (
                      <div className="mb-4">
                        <div className="text-xs font-medium text-[var(--muted)] uppercase tracking-wide mb-2">Nice to Have</div>
                        <ul className="space-y-1">
                    ...
                    {role.teamInfo && (
                      <div className="pt-3 border-t border-[var(--border)]">
                        <div className="text-xs font-medium text-[var(--muted)] uppercase tracking-wide mb-1">Team</div>
                        <p className="text-sm text-[var(--muted)]">{role.teamInfo}</p>
                      </div>
                    )}
