 ...
 * - Title (h1) and subtitle ([data-subtitle]) get GSAP typing effect.
 * - After typing finishes, only the first block fades in.
 * - Remaining blocks fade in individually on scroll.
 * - Fixed scroll-hint chevron at viewport bottom, own z-layer.
 */
export function AboutReveal({ children, className }: AboutRevealProps) {
  ...
  useEffect(() => {
    ...
    // ── 1. Typing effect for title and subtitle ──────────
    ...
    if (h1) {
      ...
      h1.textContent = ''
      titleDuration = h1Text.length * 0.035
      tweens.push(
        gsap.to(h1, {
    ...
    if (subtitle) {
      ...
      tweens.push(
        gsap.to(subtitle, {
          duration: subtitleText.length * 0.025,
    ...
    // ── 2. Hide all blocks initially ─────────────────────
    const blocks = Array.from(el.querySelectorAll('section, .fade-block'))
    ...
    const setupTimer = setTimeout(() => {
      ...
      // Remaining blocks: scroll-triggered
    ...
    return () => {
      ...
      // HMR safety: restore text and visibility
  ...
  return (
    <>
      ...
      {/* Fixed scroll hint — own z-layer, viewport-bottom anchored */}
      <div
        ref={hintRef}
