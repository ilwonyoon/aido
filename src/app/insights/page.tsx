    ...
    <div>
        ...
        <div className="space-y-6 mb-8">
          {featured.map((article) => (
            <Link key={article.slug} href={`/insights/${article.slug}`}>
              <div className="card p-6 hover:border-[var(--muted)] transition-colors cursor-pointer">
      ...
      {regular.length > 0 && (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {regular.map((article) => (
            <Link key={article.slug} href={`/insights/${article.slug}`}>
              <div className="card p-5 hover:border-[var(--muted)] transition-colors cursor-pointer">
                <div className="flex items-center gap-2 mb-3">
                  <span className="badge badge-accent text-xs">
                    {article.category.charAt(0).toUpperCase() +
                      article.category.slice(1)}
